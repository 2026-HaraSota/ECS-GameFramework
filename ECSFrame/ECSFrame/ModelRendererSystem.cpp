	...
	:SystemBase(entityOperator, systemDistributor, chunkProvider) 
{
	...
	m_default_vertex_shader->Load("Assets/Shader/VS_WorldPos.cso");
	m_default_pixel_shader->Load("Assets/Shader/PS_RimLight.cso");
...
void ModelRendererSystem::UpdateSystem(float deltaTime)
{
	...
	// 定数バッファに渡すカメラの情報
	DirectX::XMFLOAT3 camPos = main_camera->GetPosition();
	...
	for (auto& chunk : chunks)
	{
		...
		for (int i = 0; i < num_entities; ++i)
		{
			...
			m_current_pixel_shader->WriteBuffer(0, light);
			m_current_pixel_shader->WriteBuffer(1, camera);
			m_current_pixel_shader->Bind();
