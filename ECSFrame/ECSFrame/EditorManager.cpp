...
void EditorManager::InitEditor()
{
	...
	// ウィンドウ作成
	RegisterWindow<WorldWindow>(WorldWindow::WORLD_WINDOW_TITLE);
	RegisterWindow<DetailsWindow>(DetailsWindow::DETAILS_WINDOW_TITLE);
	RegisterWindow<TextInputDialog>(TextInputDialog::TEXT_INPUT_DIALOG_TITLE);
	RegisterWindow<DebugSystemOptionWindow>(DebugSystemOptionWindow::DEBUG_SYSTEM_OPTION_WINDOW_TITLE);
}
...
void EditorManager::UpdateEditor(float deltaTime)
{
	...
	CameraSystemBase* main_camera = camera_manager->GetCamera(CameraManagerSystem::CameraType::MAIN_CAMERA);
	...
	{
		CopySelectEntity();
		WorldWindow* world_window = reinterpret_cast<WorldWindow*>(GetWindow(WorldWindow::WORLD_WINDOW_TITLE));
	...
	if (IsKeyPress(VK_CONTROL) && IsKeyTrigger('R'))
	{
		ReloadResource();
		WorldWindow* world_window = reinterpret_cast<WorldWindow*>(GetWindow(WorldWindow::WORLD_WINDOW_TITLE));
		DetailsWindow* details_window = reinterpret_cast<DetailsWindow*>(GetWindow(DetailsWindow::DETAILS_WINDOW_TITLE));
		world_window->SetNeedUpdate(true);
	...
	// fps表示用にフレーム時間を渡す
	m_gui_manager->SetDeltaTime(deltaTime);
}
...
{
	// 描画
	m_system_manager->AddRenderSystem<SpriteRenderSystem>(SystemManager::DrawLayer::UI, false,
		SpriteRenderSystem::SPRITE_RENDERER_SYSTEM);
	m_system_manager->AddRenderSystem<ModelRendererSystem>(SystemManager::DrawLayer::OBJECT, false,
		ModelRendererSystem::MODEL_RENDERER_SYSTEM);
	m_system_manager->AddRenderSystem<GridRendererSystem>(SystemManager::DrawLayer::OBJECT, true,
		GridRendererSystem::GRID_RENDERER_SYSTEM);
	m_system_manager->AddRenderSystem<CollisionRendererSystem>(SystemManager::DrawLayer::OBJECT, true,
		CollisionRendererSystem::COLLISION_RENDERER_SYSTEM);
	m_system_manager->AddRenderSystem<GeometryRendererSystem>(SystemManager::DrawLayer::OBJECT, false,
		GeometryRendererSystem::GEOMETRY_RENDER_SYSTEM);
	
	// 物理
	m_system_manager->AddSystem<BoxCollisionSystem>(SystemManager::UpdateProcessType::PHYSICS, false,
		BoxCollisionSystem::BOX_COLLISION_SYSTEM);

	// カメラ
	m_system_manager->AddSystem<CameraManagerSystem>(SystemManager::UpdateProcessType::NONE, true,
	...
	camera_manager->ChangeCamera(CameraManagerSystem::CameraType::MAIN_CAMERA, CameraDccSystem::CAMERA_DCC_SYSTEM);
}
...
void EditorManager::ChangeEditMode(EditMode editMode)
{
	...
	case EditMode::PLAY:
		SaveWorld();
		WorldManager::Instance().ActivateWorld(m_edit_world_name, WorldActivateHandle::ActivateMode::CHANGE, false);
...
void EditorManager::LoadWorld()
{
	...
	for (int i = 0; i < y; ++i)
	{
		for (int j = 0; j < x; ++j)
		{
			uint32_t id = CreateEmptyEntity();
			m_entity_manager->AddComponentData(id, ComponentMask<ModelHandle>::bit);
			ModelHandle* model_handle = reinterpret_cast<ModelHandle*>(
				m_entity_manager->GetComponentData(id, ComponentMask<ModelHandle>::bit)
				);
			strcpy_s(model_handle->modelName, "LowPoly/Buildings/Building_1.fbx");
			Transform* transform = reinterpret_cast<Transform*>(
				m_entity_manager->GetComponentData(id, ComponentMask<Transform>::bit)
				);
			transform->position = DirectX::XMFLOAT3(i * span, 0.0f, j * span);
...
* @brief 空のエンティティを作成
*/
uint32_t EditorManager::CreateEmptyEntity()
{
...
/**
* @brief 選択中のサブシーンをコピーする
*/
void EditorManager::CopySubScene()
{
	// 以前コピーしたデータを解放
	if (!m_copy_subscene_data.empty())
	{
	...
	m_copy_subscene_name = m_select_subscene_name;
}
...
/**
* @brief コピー中のサブシーンをワールド内に複製
*/
void EditorManager::PasteSubScene()
{
	if (m_copy_subscene_data.empty())return;

	// サブシーン作成
	std::string unique_subscene_name = m_subscene_manager->AddSubscene(m_copy_subscene_name);

	// 新しく作成したサブシーンにエンティティを追加
	for (auto& copy_entity_data : m_copy_subscene_data)
	{
		// コピーしたデータを基にエンティティを作成
		uint32_t id = m_entity_manager->CreateLoadedEntity(copy_entity_data);
		// サブシーンに追加
		m_subscene_manager->AddEntityToSubscene(unique_subscene_name, id);
	}
}
