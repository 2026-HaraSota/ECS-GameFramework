...
public:
	...
	* @param[in] overWriteMode 書き換えタイミングオプション
	*/
	GuiInputInt* CreateGuiInputInt(std::string label, std::string id, int* outputBuffer, 
		GuiInputBase::OverWriteMode overWriteMode = GuiInputBase::OverWriteMode::ALWAYS) {
	...
	*/
	GuiInputFloat* CreateGuiInputFloat(std::string label, std::string id, float* outputBuffer,
		int decimalPlaces = GuiInputFloat::DEFAULT_DECIMAL_PLACES, 
		GuiInputBase::OverWriteMode overWriteMode = GuiInputBase::OverWriteMode::ALWAYS) {
	...
	*/
	void SetDeltaTime(float deltaTime) {
		m_draw_fps_gui.UpdateElapsedTime(deltaTime);
	}
