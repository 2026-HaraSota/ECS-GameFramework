	...
	for (auto& chunk : chunks)
	{
		...
		for (int i = 0; i < num_entities; ++i)
		{
			if (transform->IsAble() && transform != nullptr)
			{
				...
				// 形に応じて描画
				GeometryType geometry_type = static_cast<GeometryType>(solid->geometryNum);
				switch (geometry_type)
				{
				case GeometryType::BOX:		Geometory::DrawBox(); break;
				case GeometryType::SPHERE:	Geometory::DrawSphere(); break;
				default:
					break;
				}
