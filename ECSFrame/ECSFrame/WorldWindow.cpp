...
void WorldWindow::UpdateWindow()
{
	...
	m_window->AddChildren(
		...
		gui_manager->CreateGuiButton("Play", "id", 
			[&]()
			{
				if (m_editor_manager->GetEditMode() == EditorManager::EditMode::PLAY)
				{
					m_editor_manager->ChangeEditMode(EditorManager::EditMode::EDIT);
				}
				else
				{
					m_editor_manager->ChangeEditMode(EditorManager::EditMode::PLAY);
				}
	...
	for (const auto& subscene : subscene_to_entities)
	{
		...
		GuiTreeNode* subscene_node = gui_manager->CreateGuiTreeNode(subscene.first, "id", true,
			[&, gui_manager, subscene]() // 左クリック
			{
				...
				// 詳細ウィンドウにサブシーンを表示
				UpdateDetail(DetailsWindow::DetailTargetType::SUBSCENE);
			},
			...
			subscene_node->AddChild(
				gui_manager->CreateGuiSelectable(entity_name->name, "id",
					[&,entity]()
					{
						...
						UpdateDetail(DetailsWindow::DetailTargetType::ENTITY);
					})
			);
		}
...
void WorldWindow::CreateWorldMenu()
{
	...
	world_menu->AddChildren(
		...
		gui_manager->CreateGuiSelectable("Create World", "id",
			[&, world_menu]()
			{
				OpenTextInputDialog(TextInputDialog::InputMode::CREATE_WORLD);
				world_menu->ClosePopup();
			}),
		// ３．アセットから指定のワールドをロード
		gui_manager->CreateGuiSelectable("Load World", "id",
			[&, world_menu]()
			{
				OpenTextInputDialog(TextInputDialog::InputMode::LOAD_WORLD);
				world_menu->ClosePopup();
			}),
		// ４．編集中のワールドの名前を変更
		gui_manager->CreateGuiSelectable("Rename World", "id",
			[&, world_menu]()
			{
				OpenTextInputDialog(TextInputDialog::InputMode::RENAME_WORLD);
				world_menu->ClosePopup();
			}),
		...
		gui_manager->CreateGuiSelectable("Load SubScene", "id", 
			[&,world_menu]()
			{
				OpenTextInputDialog(TextInputDialog::InputMode::LOAD_SUBSCENE);
				world_menu->ClosePopup();
			}),
		// ６. 新しくサブシーンを作成
		gui_manager->CreateGuiSelectable("Create SubScene", "id", 
			[&, world_menu]()
			{
				...
				OpenTextInputDialog(TextInputDialog::InputMode::CREATE_SUBSCENE);
				world_menu->ClosePopup();
			}),
		// ７．リソースをリロード
...
void WorldWindow::CreateSubSceneMenu()
{
	...
	subscene_menu->AddChildren(
		...
		gui_manager->CreateGuiSelectable("Rename", "id", 
			[&, subscene_menu]()
			{
				OpenTextInputDialog(TextInputDialog::InputMode::RENAME_SUBSCENE);
				m_need_update = true;
				subscene_menu->ClosePopup();
			}),
		...
		// ここにサブシーンコピー・ペーストを追加
		gui_manager->CreateGuiSelectable("Copy Subscene", "id",
			[&, subscene_menu]()
			{
				m_editor_manager->CopySubScene();
				subscene_menu->ClosePopup();
			}),
		gui_manager->CreateGuiSelectable("Paste Subscene", "id",
			[&, subscene_menu]()
			{
				m_editor_manager->PasteSubScene();
				subscene_menu->ClosePopup();
			}),
